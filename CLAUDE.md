...
- **表示用URL**: 相対パス（`./filename`）でCORS制限を回避
- **コピー用URL**: 絶対URL（`https://domain/path/filename`）で外部利用可能
- **検索結果での直接操作**: カテゴリページ経由なしで即座にURLコピー・ダウンロード
...
- **機能**:
  ...
  - 検索結果ハイライト表示
  - カテゴリ別表示
  - **検索結果アクション**: 各アイコンに直接URLコピー・ダウンロードボタン
