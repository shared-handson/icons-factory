...
- **機能**: URL コピー、強制ダウンロード、レスポンシブ UI
- **対応形式**: PNG 画像のみ（SVGは自動変換後削除）
- **自動化**: フォルダ名からページタイトルを自動生成
- **ダウンロード**: fetch + blob方式で強制ダウンロード（ブラウザ表示回避）
...
1. SVGファイルを該当フォルダに配置
2. `convert-svg-to-png.yml` が自動実行:
   - Inkscapeで512x512pxのPNGに変換
   - 変換成功後に元SVGファイルを自動削除
   - 変更をコミット・プッシュ
3. `generate-icon-list.yml` が連鎖実行:
   - アイコンリスト（icons.json）を更新
   - PNGファイルのみを対象（SVGは既に削除済み）
4. GitHub Pages で自動デプロイ

**重要**: SVGファイルは変換後に自動削除されるため、リポジトリにはPNGファイルのみが残ります。

### 新しいカテゴリ追加

1. `cp -r template/ [カテゴリ名]/` でテンプレートコピー
2. 不要ファイル削除（sample-icon.png、README.md 等）
...
3. GitHub Actions が自動実行:
   - SVGがある場合: PNG変換後SVG削除
   - `icons.json`自動更新（PNGのみ対象）
4. GitHub Pages で自動デプロイ完了

### ダウンロード機能の仕様

- **強制ダウンロード**: fetch + blob方式でブラウザ表示を回避
- **相対パス使用**: CORS制限を回避
- **エラーハンドリング**: 失敗時の適切な通知
- **メモリ管理**: オブジェクトURLの自動クリーンアップ
